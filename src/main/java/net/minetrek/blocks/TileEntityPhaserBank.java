...
public class TileEntityPhaserBank extends TileEntity implements SimpleComponent, ISidedInventory, IUpdatePlayerListBox {
	...
	public boolean isItemValidForSlot(int i, ItemStack itemstack) {
		return itemstack == null || itemstack.getItem() == MineTrekItems.phaser;
	}
	@Override
	public String getComponentName() {
		return "minetrek_phaser_bank";
	}
	...
	public Object[] fire(Context context, Arguments args) {
		BlockPhaserBank block = (BlockPhaserBank)this.worldObj.getBlockState(this.pos).getBlock();
		...
		if(this.inventory[0].getItemDamage() >= this.inventory[0].getMaxDamage()){
			return new Object[] {false, "Phaser out of Power"};
		}
	...
	public void shoot(World world, int x, int y, int z, int fromX, int fromY, int fromZ) {
		EntityPhaserBolt e = new EntityPhaserBolt(world, fromX, fromY, fromZ);
