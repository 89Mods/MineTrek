...
public class WorldGenDecoration extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos start) {
		...
		for (int l = clusterSize; l --> 0; ) {
			int x = start.getX() + rand.nextInt(xVar) - rand.nextInt(xVar);
			int y = start.getY() + (yVar > 1 ? rand.nextInt(yVar) - rand.nextInt(yVar) : 0);
			int z = start.getZ() + rand.nextInt(zVar) - rand.nextInt(zVar);
			...
			if ((!seeSky || chunk.canSeeSky(new BlockPos(x,y,z))) &&
				...
				do {
					if (!checkStay || block.block.canPlaceBlockAt(world, new BlockPos(x,y,z)))
						r |= world.setBlockState(new BlockPos(x,y,z), block.block.getStateFromMeta(block.metadata), 2);
					else break;
					++y;
