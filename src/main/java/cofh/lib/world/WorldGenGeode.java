		...
		for (x = 0; x < width; ++x) {
			for (z = 0; z < width; ++z) {
				for (y = 0; y < height; ++y) {
					...
					if (flag && !canGenerateInBlock(world, start.getX() + x, start.getY()+ y, start.getZ() + z, genBlock)) {
						return false;
					}
				}
			}
			...
			for (z = 0; z < width; ++z) {
				for (y = 0; y < height; ++y) {
					if (spawnBlock[(x * width + z) * height + y]) {
						boolean t = generateBlock(world, start.getX() + x, start.getY() + y, start.getZ() + z, cluster);
						r |= t;
						if (!t) {
			...
			for (z = 0; z < width; ++z) {
				for (y = 0; y < height; ++y) {
					if (fillBlock != null && hollowBlock[(x * width + z) * height + y]) {
						r |= generateBlock(world, start.getX() + x, start.getY() + y, start.getZ() + z, fillBlock);
					} else {
						...
						if (flag) {
							r |= generateBlock(world, start.getX() + x, start.getY() + y, start.getZ() + z, outline);
						}
					}
				}
			}
