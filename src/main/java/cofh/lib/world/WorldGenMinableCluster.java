...
public class WorldGenMinableCluster extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (blocks < 4) { // HACK: at 1 and 2 no ores are ever generated. at 3 only 1/3 veins generate
			return generateTiny(world, rand, pos.getX(), pos.getY(), pos.getZ());
		}
	...
	public static boolean canGenerateInBlock(World world, int x, int y, int z, WeightedRandomBlock[] mat) {
		...
		for (int j = 0, e = mat.length; j < e; ++j) {
			...
			if ((-1 == genBlock.metadata || genBlock.metadata == world.getBlockState(new BlockPos(x,y,z)).getBlock().getMetaFromState(world.getBlockState(new BlockPos(x,y,z)))) &&
					
					(world.getBlockState(new BlockPos(x,y,z)).getBlock() == Blocks.stone || block.isAssociatedBlock(genBlock.block))) {
				return true;
			}
		}
		return false;
