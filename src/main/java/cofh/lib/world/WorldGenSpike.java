...
public class WorldGenSpike extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos start) {
		...
		while (world.isAirBlock(start) && start.getY() > 2) {
			start = new BlockPos(start.getX(),start.getY() - 1, start.getZ());
		}

		if (!canGenerateInBlock(world, start.getX(), start.getY(), start.getZ(), genBlock)) {
			return false;
		}
		...
		if (size > 1 && positionVariance > 0) {
			start = new BlockPos(start.getX(), start.getY() + rand.nextInt(positionVariance + 1) - 1, start.getZ());
		}
			...
			for (int x = -width; x <= width; ++x) {
				...
				for (int z = -width; z <= width; ++z) {
					...
					if ((x == 0 && z == 0 || xDist * xDist + zDist * zDist <= layerSize * layerSize) &&
						...
						if (y != 0 && width > 1)
							generateBlock(world, start.getX() + x, start.getY() - y + offsetHeight, start.getZ() + z, genBlock, cluster);
					}
				}
			}
		}
