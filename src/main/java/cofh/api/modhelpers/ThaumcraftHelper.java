...
public class ThaumcraftHelper {
	...
	static {
		l: try {
			if (!net.minecraftforge.fml.common.ModAPIManager.INSTANCE.hasAPI("Thaumcraft|API"))
				break l;
	...
	private static void parseAspects(Object aspectList, String toadd) throws Throwable {
		...
		if (!toadd.isEmpty()) {
			...
			for (int i = 0, e = list.length; i < e; ++i) {
				...
				if (temp.length != 2) {
					net.minecraftforge.fml.common.FMLLog.bigWarning("[CoFH Thaumcraft Helper] Invalid aspect entry '%s'", list[i]);
					continue;
				}
				...
				if (aspects.containsKey(aspect)) {
					...
					net.minecraftforge.fml.common.FMLLog.fine("[CoFH Thaumcraft Helper] %s aspect missing.", temp[1]);
				}
			}
		}
	}
